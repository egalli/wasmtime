...
impl SingleFunctionCompiler {
    ...
    pub fn compile(&mut self, function: Function) -> Result<CompiledFunction, CompilationError> {
        ...
        let code_page = compile(function.clone(), self.isa.as_ref())?; // TODO how can we avoid this function.clone()? Context requires ownership of the function...
        ...
        let trampoline = self
            ...
            .or_insert_with(|| {
                let ir = make_trampoline(&function.signature, isa);
...
impl Trampoline {
    ...
    pub unsafe fn write_value_to(v: &DataValue, p: *mut u128) {
        match v {
    ...
    pub unsafe fn read_value_from(p: *const u128, ty: Type) -> DataValue {
        match ty {
...
impl<'a> CompiledFunction<'a> {
    ...
    pub fn call(&self, arguments: &[DataValue]) -> Vec<DataValue> {
        let mut arguments = make_arguments(arguments, &self.signature);
        let arguments_address = arguments.as_mut_ptr();
        ...
        collect_returns(&arguments, &self.signature)
    }
}
...
fn make_arguments(arguments: &[DataValue], signature: &ir::Signature) -> Vec<u128> {
...
fn collect_returns(values: &[u128], signature: &ir::Signature) -> Vec<DataValue> {
    assert!(values.len() >= signature.returns.len());
    let mut returns = Vec::with_capacity(signature.returns.len());

    // Store the argument values into `values_vec`.
    for (slot, param) in values.iter().zip(&signature.returns) {
        unsafe {
            returns.push(Trampoline::read_value_from(slot, param.value_type));
        }
    }
