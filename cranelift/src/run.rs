...
use cranelift_reader::{parse_run_command, parse_test, Details, IsaSpec, ParseOptions, RunCommand};
...
fn run_file_contents(file_contents: String) -> Result<(), String> {
    ...
    for (func, Details { comments, .. }) in test_file.functions {
            ...
            .filter(|c| RunCommand::is_potential_run_command(c.text))
        {
            ...
            let command = parse_run_command(trimmed, &func.signature).map_err(|e| e.to_string())?;
